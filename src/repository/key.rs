...
impl EncryptedKey { 
   /// Produces an encrypted key from the specified userkey and encryption method
    pub fn encrypt(key: &Key, encryption: Encryption, user_key: &[u8], scrypt_n: u8, scrypt_r: u32, scrypt_p: u32) -> EncryptedKey {
    ...
    pub fn decrypt(&self, user_key: &[u8]) -> Option<Key> {
        ...
        // Decrypt the key
        let key_bytes = self.encryption.decrypt(&self.encrypted_bytes,&generated_key_bytes)?;
        // Deserialize the key
